...
```{r}
data3 <- data2 %>% 
  mutate("forecast" = arma.forecast(data2, nstart=12, nstep=90, marima=Marima1, check = F)$forecasts[3,] ) %>% 
  ...
  mutate("forecast_inv_trans_upper" = 
           ...
           diffinv(differences = 2, xi = log(data2$NumberWords[1:2])) %>% 
           exp() %>% 
           .[1:102]) %>%  # got vector of length 104, which is strange
  mutate("residuals" = NumberWords-forecast_inv_trans) %>% 
  mutate("trans_residuals")
